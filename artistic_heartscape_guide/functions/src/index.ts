 ...
 * リクエストボディのバリデーション
 */
function validateRequest(body: unknown): { valid: true; request: GeminiRequest } | { valid: false; error: string } {
  ...
  // type の検証（必須）
  if (!req.type || !['commentary', 'insiderStory', 'timeline'].includes(req.type as string)) {
  ...
  if (req.paintings !== undefined) {
    ...
    for (const p of req.paintings) {
      if (!p || typeof p !== 'object' || typeof (p as Record<string, unknown>).title !== 'string' || typeof (p as Record<string, unknown>).year !== 'number') {
        return { valid: false, error: 'Each painting must have title (string) and year (number)' };
      }
    }
  }

  return { valid: true, request: body as GeminiRequest };
}
